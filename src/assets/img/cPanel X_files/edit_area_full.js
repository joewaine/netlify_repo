...
EAL.prototype = {
	...
	start: function(id) {
		...
		if (
			...
			(eAs[id]["settings"]["debug"] ||
				eAs[id]["settings"]["allow_toggle"])
		) {
			span = d.createElement("span");
			span.id = "EditAreaArroundInfos_" + id;
			if (eAs[id]["settings"]["allow_toggle"]) {
				checked =
					eAs[id]["settings"]["display"] == "onload"
						? "checked='checked'"
						: "";
		...
		if (
			typeof window.frames["frame_" + area["settings"]["id"]] !=
			"undefined"
		)
	...
	set_editarea_size_from_textarea: function(id, frame) {
		...
		elem = document.getElementById(id);
		width =
			Math.max(eAs[id]["settings"]["min_width"], elem.offsetWidth) + "px";
		height =
	...
	setSelectionRange: function(id, new_start, new_end) {
		var fs = window.frames;
		if (fs["frame_" + id] && eAs[id]["displayed"] == true) {
			fs["frame_" + id].editArea.area_select(
				new_start,
				new_end - new_start
			);
			if (!this.isIE) {
	...
	hide: function(id) {
		...
		if (d.getElementById(id) && !t.hidden[id]) {
			...
			if (fs["frame_" + id]) {
				...
				if (fs["frame_" + id] && eAs[id]["displayed"] == true) {
					scrollTop = fs["frame_" + id].document.getElementById(
						"result"
					).scrollTop;
					scrollLeft = fs["frame_" + id].document.getElementById(
						"result"
					).scrollLeft;
				} else {
					scrollTop = d.getElementById(id).scrollTop;
		...
		switch (cmd) {
			case "EA_init":
				if (eAs[id]["settings"]["EA_init_callback"].length > 0)
					eval(
						eAs[id]["settings"]["EA_init_callback"] +
							"('" +
							id +
							"');"
					);
			...
			case "EA_delete":
				if (eAs[id]["settings"]["EA_delete_callback"].length > 0)
					eval(
						eAs[id]["settings"]["EA_delete_callback"] +
							"('" +
							id +
							"');"
					);
			...
			case "EA_submit":
				if (eAs[id]["settings"]["submit_callback"].length > 0)
					eval(
						eAs[id]["settings"]["submit_callback"] +
							"('" +
							id +
							"');"
					);
			...
			if (strTagName != "undefined") {
				...
				if (
					(strTagName.toLowerCase() == elem_type.toLowerCase() ||
						elem_type == "") &&
					(elem_attribute == "" ||
						child_attribute == elem_attribute_match)
				) {
					if (option == "all") {
	...
	if (t && t.focused) {
		...
		try {
			...
			if (stored_range.parentElement() == t) {
				...
				relative_top =
					range.offsetTop - calculeOffsetTop(t) + scrollTop;
				...
				tab = t.value
					.substr(0, range_start + range.text.length)
					.split("\n");
...
EAL.prototype.resize_area = function(e) {
	...
	if (allow == "both" || allow == "y") {
		...
		document.getElementById("edit_area_resize").style.height =
			newHeight + "px";
	}
	if (allow == "both" || allow == "x") {
		newWidth = Math.max(20, getMouseX(e) - eAL.resize["start_left"]);
		document.getElementById("edit_area_resize").style.width =
			newWidth + "px";
	}
	return false;
};
		...
		if (!this.syntax[lang]) {
			...
			if (this.load_syntax[lang]["KEYWORDS"]) {
				...
				for (var i in this.load_syntax[lang]["KEYWORDS"]) {
					if (
						typeof this.load_syntax[lang]["KEYWORDS"][i] ==
						"function"
					)
						continue;
			...
			if (this.load_syntax[lang]["OPERATORS"]) {
				...
				for (var i in this.load_syntax[lang]["OPERATORS"]) {
					if (
						typeof this.load_syntax[lang]["OPERATORS"][i] ==
						"function"
					)
						continue;
					if (nb > 0) str += "|";
			...
			if (this.load_syntax[lang]["DELIMITERS"]) {
				...
				for (var i in this.load_syntax[lang]["DELIMITERS"]) {
					if (
						typeof this.load_syntax[lang]["DELIMITERS"][i] ==
						"function"
					)
						continue;
					if (nb > 0) str += "|";
			...
			if (this.load_syntax[lang]["QUOTEMARKS"]) {
				for (var i in this.load_syntax[lang]["QUOTEMARKS"]) {
					if (
						typeof this.load_syntax[lang]["QUOTEMARKS"][i] ==
						"function"
					)
						continue;
			...
			if (this.load_syntax[lang]["COMMENT_SINGLE"]) {
				for (var i in this.load_syntax[lang]["COMMENT_SINGLE"]) {
					if (
						typeof this.load_syntax[lang]["COMMENT_SINGLE"][i] ==
						"function"
					)
						continue;
					var x = this.get_escaped_regexp(
			...
			if (this.load_syntax[lang]["COMMENT_MULTI"]) {
				for (var i in this.load_syntax[lang]["COMMENT_MULTI"]) {
					if (
						typeof this.load_syntax[lang]["COMMENT_MULTI"][i] ==
						"function"
					)
						continue;
			...
			if (this.load_syntax[lang]["SCRIPT_DELIMITERS"]) {
				...
				for (var i in this.load_syntax[lang]["SCRIPT_DELIMITERS"]) {
					if (
						typeof this.load_syntax[lang]["SCRIPT_DELIMITERS"][i] ==
						"function"
					)
						continue;
					this.syntax[lang]["script_delimiters"][
			...
			if (this.load_syntax[lang]["REGEXPS"]) {
				for (var i in this.load_syntax[lang]["REGEXPS"]) {
					if (
						typeof this.load_syntax[lang]["REGEXPS"][i] ==
						"function"
					)
						continue;
			...
			if (this.load_syntax[lang]["STYLES"]) {
				...
				for (var i in this.load_syntax[lang]["STYLES"]) {
					if (
						typeof this.load_syntax[lang]["STYLES"][i] == "function"
					)
						continue;
					if (
						typeof this.load_syntax[lang]["STYLES"][i] != "string"
					) {
						for (var j in this.load_syntax[lang]["STYLES"][i]) {
							lang_style[lang][j] = this.load_syntax[lang][
								"STYLES"
							][i][j];
						}
					} else {
						lang_style[lang][i] = this.load_syntax[lang]["STYLES"][
							i
						];
					}
				}

...
function check_for_encoding_change(template, data) {
	...
	if (saved_charset.normalize_charset() !== CHARSET.normalize_charset()) {
		var message = YAHOO.lang.substitute(template, {
		...
		enc_dialog.setHeader(
			"<div class='lt'></div><span>" +
				LEXICON.charset_changed +
...
function confirm_close(clicked_el) {
	...
	if (to_confirm) {
		...
		confirmation.beforeShowEvent.subscribe(
			confirmation.center,
			confirmation,
			true
		);
		confirmation.show(clicked_el);
	} else {
		window.close();
	}
}
